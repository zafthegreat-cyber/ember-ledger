...
Phase 2C-A adds a separate versioned `code3.purchase-receiving.v1` browser source for non-authoritative Purchase Drafts, owner-confirmed exact-money Purchases, append-only Purchase/Receiving history, and bounded activity. Phase 2C-B keeps Inventory Handoff Preview and Inventory Creation Candidate as derived in-memory projections, then permits one explicit verified-OWNER confirmation into the existing local Business Inventory. Phase 2C-C and Phase 2C-D advance that existing document through typed append-only adjustments and reconciliation events while preserving original Sale facts. Phase 2C-E adds only an ephemeral `AccountantReviewItem` projection over those records; it adds no schema, collection, accounting ledger, backup source, or migration path. `LOCAL_ONLY` remains authoritative.
...
### Phase 2C-B creation, Phase 2C-C correction/disposition, and Phase 2C-D reconciliation records
...
The existing Flip Scout/Business Inventory document retains storage key `ember-and-tide.flip-scout.v1`. Phase 2C-D advances it to schema version 5 without adding another Inventory or accounting authority:

| Collection | Current purpose |
|---|---|
| `inventory` | existing canonical local Business Inventory items; provenance-managed rows retain immutable creation facts plus validated current product/condition/disposition/quantity/cost state |
...
| `inventoryAdjustments` | typed append-only reviewed correction, disposition, physical-return, cost, and reversal records; never destructive replacement of creation facts |
| `inventoryReconciliationEvents` | append-only confirmed historical Sale/COGS/product reconciliation facts; Transfer effects remain empty/blocked until managed Transfer authority exists |
...
`inventoryAdjustments` preserve all post-creation mutation as an ordered, typed, append-only chain. Phase 2C-C previews and explicitly confirms supported whole-lot product/condition corrections, physical returns and quantity dispositions bounded by unsold/untransferred availability, pre-sale/pre-transfer acquisition-cost correction, and eligible reversal repair. Every adjustment carries deterministic identity, idempotency/request facts, before/after state, exact effects, bounded reason, and creation-bundle references. Original Purchase, Receiving, creation, sale, and transfer evidence is not rewritten.

`inventoryReconciliationEvents` preserve post-sale historical reconciliation without altering completed Sales. Each confirmed event has deterministic identity, a typed category/status, source correction and optional prior-reconciliation references, Inventory/lot/Purchase/Receiving relationships, bounded immutable Sale references, empty transfer references until a canonical managed-transfer authority exists, original/corrected projections, exact signed realized-COGS and remaining-Inventory deltas, owner confirmation, provenance, and a semantic digest. Candidates/previews remain in memory only.

Managed Sales retain their repository-assigned `inventoryAllocationSequence`, Sale-time product snapshot, exact recorded COGS, profit, and ROI. Effective reporting is derived as the immutable original plus confirmed reconciliation deltas. For partially sold lots, the sold prefix and remaining suffix use the original deterministic unit order; their deltas must sum exactly to the lot-cost change. A negative signed COGS adjustment is valid, but acquisition cost itself remains a non-negative integer minor-unit value.

Transfer reconciliation categories exist only as fail-closed review vocabulary. Schema 5 still has no canonical managed-transfer collection or writer, so no event may claim a confirmed transfer effect. Legacy Collection-to-Forge movement remains a separate authority.

A refund does not imply a physical return. Structured quantity-correction reason is canonical event metadata and determines returned/lost/disposed semantics; `RETURN_TO_RETAILER` consumes every currently available unit while `PARTIAL_RETURN` remains the bounded partial category. Replacements require a scoped Purchase Event linked one-to-one with an effective unreversed physical-return adjustment, a new Receiving Event, and a separate Inventory-creation review. The new bundle stores its authorization/return dependency, deterministic source-unit offset, and exact returned unit-cost vector; validation rejects overlapping or altered slices, and a completed replacement blocks reversal of the consumed return. Unexpected extras require a separate acquisition/cost review. The Phase 2C-C correction gateway blocks product/condition and cost correction after any Sale or Transfer; supported post-sale effects instead pass through the separate Phase 2C-D reconciliation boundary. Raw card and Graded card remain outside the generic correction model pending a type-specific card/slab condition contract. Provenance-managed items/lots cannot use generic edit/delete/import paths, and the private recovery journal is not a domain collection or backup source.

Backup validation requires a complete application/event/item/lot/adjustment/reconciliation chain with strict identity, sequence, current/original state, quantity, immutable Sale snapshots, and exact-cost reconciliation. The mixed `deal-finder.inventory` migration path, five provenance/reconciliation paths, and managed Sales remain `REQUIRES_MAPPING`. See [INVENTORY_CREATION_CONTRACT.md](./INVENTORY_CREATION_CONTRACT.md), [INVENTORY_CORRECTION_DISPOSITION_CONTRACT.md](./INVENTORY_CORRECTION_DISPOSITION_CONTRACT.md), and [INVENTORY_RECONCILIATION_CONTRACT.md](./INVENTORY_RECONCILIATION_CONTRACT.md).

#### Phase 2C-E Accountant Review projection

`AccountantReviewItem` is a frozen, non-authoritative view derived after verified OWNER authorization. It may reference one immutable managed Sale, its acquisition lot and Purchase/Receiving provenance, and one confirmed reconciliation effect. Safe projection fields include derived item identity, typed review category, attention level, `FILING_STATUS_UNKNOWN`, Sale and reconciliation references/dates, original and correction month/quarter/year, exact original/adjustment/effective COGS, exact original/effective profit when exact proceeds exist, product reclassification, refund-versus-physical-return classification, and bounded review warnings.

The period model treats a Sale `YYYY-MM-DD` as recorded and classifies an ISO reconciliation instant by its UTC calendar date. This is an explicit technical basis, not an authoritative owner-business time-zone or tax-period policy. Invalid calendar input is rejected rather than rolled over.

Review rows, filters, groups, and summaries exist only in memory. They are not stored in `code3.purchase-receiving.v1`, `ember-and-tide.flip-scout.v1`, localStorage, Backup Format v1, Restore Preview, Migration Preview, Upstash, Supabase, or a remote API. Original Sale COGS is counted once, affected-Sale deltas are counted once, and reversal rows remain visible while their signed effects net. `Accountant Review != Accounting Mutation`; no journal entry, tax filing/amendment, export, or note entity is introduced. See [ACCOUNTANT_REVIEW_CONTRACT.md](./ACCOUNTANT_REVIEW_CONTRACT.md).
