...
Phase 2C-A adds a separate OWNER-gated Purchase/Receiving domain. It recursively rejects browser authority, authentication/payment/retailer/provider/proxy secrets, and raw evidence before persistence; confirms a Purchase only through an explicit versioned owner action; and keeps Receiving and Inventory separate. It does not connect upstream evidence or expose an Inventory writer.
...
| Inventory handoff isolation | Phase 2C-A local implementation | preview is derived in memory, absent from backup/migration/persistence, and no Inventory writer is imported or called |
...
A complete future recovery point includes schema/version manifest, canonical records, provenance, audit references, protected file manifest/content, counts, hashes, and validation results. Phase 1A covers registered browser sources; Phase 1B can additionally include the bounded canonical export only when it is owner-authorized and valid. Phase 2A raised the registry to 22 sources (18 locally included and four excluded/conditional), Phase 2B1 to 23/19, Phase 2D-A to 24/20, and Phase 2C-A to 25 sources with 21 locally included by registering validated nonsecret `code3.purchase-receiving.v1` metadata. Plaintext passwords, OTPs, tokens, sessions, OAuth state/codes/verifiers, provider/Bot/retailer/payment/proxy secrets, raw/protected/provider content, credential-bearing URLs, security links, and owner-authority fields remain prohibited. Coverage must say `PARTIAL` whenever a relevant server source or file byte is absent. Restore Preview remains bounded and zero-write, including Account Ops, Inbox/Order, Bot Operations, and Purchase/Receiving schema/count/ID/reference checks; all Purchase/Receiving paths are `REQUIRES_MAPPING`, Inventory Handoff Preview is excluded, and applying restore is not implemented. Any expanded server export, future restore, or deletion operation requires fresh owner authorization and rate limiting.
...
| Delivery, refund, or receiving silently mutates Inventory | Phase 2C-A keeps Receiving Events append-only and exposes only an Inventory Handoff Preview | maintain no Inventory writer/import; require separate owner-confirmed inventory phase with reversal and audit design |
