...
Phase 2C-A adds a separate OWNER-gated Purchase/Receiving domain. Published Phase 2C-B adds one explicit local Inventory creation boundary while retaining `Receiving != Inventory` and `Inventory Creation Candidate != Inventory`. Published Phase 2C-C adds a separate preview-first correction/disposition boundary with `Inventory Correction Candidate != Inventory Mutation` and `Refund != Physical Return`. Published Phase 2C-D adds `Inventory Reconciliation Candidate != Historical Mutation`: immutable Sale/Transfer facts remain unchanged while confirmed exact signed deltas become separate append-only projections. Phase 2C-E adds `Accountant Review != Accounting Mutation`: OWNER-only period/financial projections remain ephemeral and read-only. No phase connects upstream evidence automatically, posts accounting/tax data, or weakens an existing confirmation boundary.
...
| Inventory handoff/candidate isolation | Phase 2C-B through 2C-D published | handoff, creation, correction, and reconciliation previews/candidates are derived in memory and excluded from backup/migration/persistence; none can mutate Inventory/history without separate confirmation |
| Inventory OWNER confirmation | Phase 2C-B through 2C-D published | OWNER check before Inventory access; strict allowlists; authoritative re-derivation inside Web Lock; expected versions, stable identity, journal/write/read-back and conflict failure |
| Inventory acquisition provenance | Phase 2C-D published | schema-5 item/lot/application/event/typed-adjustment/reconciliation chains strictly reconcile immutable creation/Sale facts, current state, quantity and exact cost; generic edit/delete/import blocked |
| Inventory correction/disposition safety | Phase 2C-C published | full/partial physical-return categories have distinct bounded semantics; quantity reason is typed; refund alone never removes Inventory; replacement Receiving requires an effective unreversed return, is resumable after a noted-only interruption, persists its dependency, blocks consumed-return reversal, and still requires separate Inventory confirmation; extras remain blocked; Raw/Graded deferred |
| Historical Sale/COGS reconciliation | Phase 2C-D published | immutable Sale snapshots plus ordered signed deltas; exact sold/remaining/total cost conservation; Sale bytes and allocation sequence preserved; transaction and correction timestamps separated |
| Accountant Review isolation | Phase 2C-E local candidate | verified OWNER before reads; frozen in-memory period/exact-money projections; disclosed UTC event-date basis and `FILING_STATUS_UNKNOWN`; no repository, ledger, journal/tax action, export, notes, backup, migration, provider, or network path |
| Transfer reconciliation | NOT_CONFIGURED | no canonical managed-transfer writer exists; transfer categories and multi-hop cases fail closed/require future reviewed authority rather than promoting legacy movement |
...
Phase 2C-E does not reduce this exposure by copying financial history into another store. Accountant Review is computed after verified OWNER authorization and discarded with the mounted view. Filters, groups, review flags, filing-status placeholders, and exact original/effective summaries stay in memory; there is no localStorage key, telemetry payload, accountant export, note store, remote request, or mutation method. It makes no tax/legal claim and cannot be used as evidence that a return was filed or amended.
...
A complete future recovery point includes schema/version manifest, canonical records, provenance, audit references, protected file manifest/content, counts, hashes, and validation results. Phase 1A covers registered browser sources; Phase 1B can additionally include the bounded canonical export only when it is owner-authorized and valid. Phase 2A raised the registry to 22 sources (18 locally included and four excluded/conditional), Phase 2B1 to 23/19, Phase 2D-A to 24/20, and Phase 2C-A to 25/21. Phase 2C-D keeps those totals and extends the existing Deal Finder source to schema 5 with validated nonsecret Inventory item/lot/application/event/typed-adjustment/reconciliation metadata and immutable managed Sales. Phase 2C-E keeps the same totals because Accountant Review is regenerated, not stored. Backup validation requires complete, strictly reconciled creation, adjustment, and reconciliation chains. Plaintext passwords, OTPs, tokens, sessions, OAuth state/codes/verifiers, provider/Bot/retailer/payment/proxy secrets, raw/protected/provider content, credential-bearing URLs, security links, owner-authority fields, Inventory handoff/creation/correction/reconciliation previews or candidates, Accountant Review items/filters/summaries, and the private Inventory journal remain prohibited. Coverage must say `PARTIAL` whenever a relevant server source or file byte is absent. Restore Preview remains bounded and zero-write; the mixed `deal-finder.inventory`, Sale, Inventory provenance, and all Purchase/Receiving paths are `REQUIRES_MAPPING`, and applying restore is not implemented. Any expanded server export, future restore, or deletion operation requires fresh owner authorization and rate limiting.
...
| Accountant Review is treated as a posted accounting/tax fact or leaks business history | Phase 2C-E derives sensitive financial projections from browser-local canonical records | verified OWNER before reads; in-memory-only projections; no export/notes/network/writer; separate original/effective labels; `FILING_STATUS_UNKNOWN`; cautious non-tax-advice copy; exact once-only aggregation and security tests |
