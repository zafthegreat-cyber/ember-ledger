...
Phase 2C-A adds a separate OWNER-gated Purchase/Receiving domain. Published Phase 2C-B adds one explicit local Inventory creation boundary while retaining `Receiving != Inventory` and `Inventory Creation Candidate != Inventory`. Published Phase 2C-C adds a separate preview-first correction/disposition boundary with `Inventory Correction Candidate != Inventory Mutation` and `Refund != Physical Return`. Phase 2C-D adds `Inventory Reconciliation Candidate != Historical Mutation`: immutable Sale/Transfer facts remain unchanged while confirmed exact signed deltas become separate append-only projections. Every boundary requires verified OWNER, re-derives current state inside the same-origin exclusive lock, verifies read-back, and fails closed on stale/conflicting state. None connects upstream evidence or mutates Inventory automatically.
...
| Inventory handoff/candidate isolation | Phase 2C-B/C published / Phase 2C-D local candidate | handoff, creation, correction, and reconciliation previews/candidates are derived in memory and excluded from backup/migration/persistence; none can mutate Inventory/history without separate confirmation |
| Inventory OWNER confirmation | Phase 2C-B/C published / Phase 2C-D local candidate | OWNER check before Inventory access; strict allowlists; authoritative re-derivation inside Web Lock; expected versions, stable identity, journal/write/read-back and conflict failure |
| Inventory acquisition provenance | Phase 2C-D local candidate | schema-5 item/lot/application/event/typed-adjustment/reconciliation chains strictly reconcile immutable creation/Sale facts, current state, quantity and exact cost; generic edit/delete/import blocked |
...
| Historical Sale/COGS reconciliation | Phase 2C-D local candidate | immutable Sale snapshots plus ordered signed deltas; exact sold/remaining/total cost conservation; Sale bytes and allocation sequence preserved; prior-period and correction timestamps separated |
...
A complete future recovery point includes schema/version manifest, canonical records, provenance, audit references, protected file manifest/content, counts, hashes, and validation results. Phase 1A covers registered browser sources; Phase 1B can additionally include the bounded canonical export only when it is owner-authorized and valid. Phase 2A raised the registry to 22 sources (18 locally included and four excluded/conditional), Phase 2B1 to 23/19, Phase 2D-A to 24/20, and Phase 2C-A to 25/21. Phase 2C-D keeps those totals and extends the existing Deal Finder source to schema 5 with validated nonsecret Inventory item/lot/application/event/typed-adjustment/reconciliation metadata and immutable managed Sales. Backup validation requires complete, strictly reconciled creation, adjustment, and reconciliation chains. Plaintext passwords, OTPs, tokens, sessions, OAuth state/codes/verifiers, provider/Bot/retailer/payment/proxy secrets, raw/protected/provider content, credential-bearing URLs, security links, owner-authority fields, Inventory handoff/creation/correction/reconciliation previews or candidates, and the private Inventory journal remain prohibited. Coverage must say `PARTIAL` whenever a relevant server source or file byte is absent. Restore Preview remains bounded and zero-write; the mixed `deal-finder.inventory`, Sale, Inventory provenance, and all Purchase/Receiving paths are `REQUIRES_MAPPING`, and applying restore is not implemented. Any expanded server export, future restore, or deletion operation requires fresh owner authorization and rate limiting.
