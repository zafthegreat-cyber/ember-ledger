...
Phase 2D-A adds an OWNER-gated, provider-neutral `LOCAL_ONLY` Bot Operations domain and UI. It rejects browser authority, Bot/retailer/payment/proxy credentials, raw provider payloads/logs, and credential-bearing URLs before hashing/persistence/backup; preserves scoped append-only event history and contradictions; and cannot create a Purchase or Inventory mutation. Hayha and Stellar remain `NOT_CONFIGURED`, all live capabilities are false, and no real provider/network/credential is used.
