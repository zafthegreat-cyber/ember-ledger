...
| Mailbox provider runtime boundary | Phase 2B1 local, unavailable by default | `backend/src/providerRuntime`, protected provider-connections router; no connect/callback/live adapter or hosted activation |
...
| Protected Inbox/order evidence | Phase 2B1 local implementation | recursive authority/secret rejection; minimization before hashing/persistence; exact-money/idempotent Order Candidates require owner review and cannot create Purchases |
...
A complete future recovery point includes schema/version manifest, canonical records, provenance, audit references, protected file manifest/content, counts, hashes, and validation results. Phase 1A covers registered browser sources; Phase 1B can additionally include the bounded canonical export only when it is owner-authorized and valid. Phase 2A raised the registry to 22 sources (18 locally included and four excluded/conditional) by adding sanitized Account Ops metadata. Phase 2B1 raises it to 23 sources (19 locally included and four excluded/conditional) by registering the separately validated `code3.inbox-order.v1` metadata source. Plaintext passwords, OTPs, tokens, sessions, OAuth state/codes/verifiers, provider secrets, raw/protected message content, security links, and owner-authority fields remain prohibited. Coverage must say `PARTIAL` whenever a relevant server source or file byte is absent. Restore Preview remains bounded and zero-write, including Account Ops and Inbox/Order schema/count/ID checks plus existing generic reference diagnostics; it does not yet validate every candidate-event/source-event relationship, and applying restore is not implemented. Any expanded server export, future restore, or deletion operation requires fresh owner authorization and rate limiting.
