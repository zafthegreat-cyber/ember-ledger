...
Published Phase 2D-A adds an OWNER-gated, provider-neutral `LOCAL_ONLY` Bot Operations domain and UI. Phase 2D-B1 adds static official-source discovery evidence and a fail-closed pilot decision without provider traffic or credentials. Browser authority, Bot/retailer/payment/proxy credentials, raw provider payloads/logs, and credential-bearing URLs remain rejected before hashing/persistence/backup. Hayha and Stellar remain `NOT_CONFIGURED`, all live capabilities are false, and no real provider/network/credential is used.
...
- Never persist Bot/provider credentials, retailer authentication values, payment credentials, proxy endpoints/authentication values, raw provider logs/payloads, or Bot client authority. Bot proxy *metadata* is distinct from proxy credentials.
...
A complete future recovery point includes schema/version manifest, canonical records, provenance, audit references, protected file manifest/content, counts, hashes, and validation results. Phase 1A covers registered browser sources; Phase 1B can additionally include the bounded canonical export only when it is owner-authorized and valid. Phase 2A raised the registry to 22 sources (18 locally included and four excluded/conditional), Phase 2B1 to 23/19, and Phase 2D-A to 24 sources with 20 locally included by registering validated nonsecret `code3.bot-ops.v1` metadata. Plaintext passwords, OTPs, tokens, sessions, OAuth state/codes/verifiers, provider/Bot/retailer/payment/proxy secrets, raw/protected/provider content, credential-bearing URLs, security links, and owner-authority fields remain prohibited. Coverage must say `PARTIAL` whenever a relevant server source or file byte is absent. Restore Preview remains bounded and zero-write, including Account Ops, Inbox/Order, and Bot Operations schema/count/ID/reference checks; all ten Bot paths are `REQUIRES_MAPPING`, and applying restore is not implemented. Any expanded server export, future restore, or deletion operation requires fresh owner authorization and rate limiting.
