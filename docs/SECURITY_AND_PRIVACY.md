...
Phase 2C-A adds a separate OWNER-gated Purchase/Receiving domain. Phase 2C-B adds one explicit local Inventory mutation boundary while retaining `Receiving != Inventory` and `Inventory Creation Candidate != Inventory`: it re-derives eligibility/product/cost inside a same-origin exclusive lock, requires verified OWNER, writes deterministic provenance records to the existing Business Inventory document, verifies read-back, and fails closed on stale/conflicting state. It does not connect upstream evidence or create Inventory automatically.
...
| Inventory handoff/candidate isolation | Phase 2C-B local candidate | preview and candidate are derived in memory and excluded from backup/migration/persistence; neither can mutate Inventory without separate confirmation |
| Inventory OWNER confirmation | Phase 2C-B locally validated candidate; unpublished | OWNER check before Inventory access; Purchase/Receiving return/cancel/missing gates; manual resolution requires existing local product relationship/reason; authoritative re-derivation inside Web Lock; exact read-back |
| Inventory acquisition provenance | Phase 2C-B locally validated candidate; unpublished | schema-3 item/lot/application/event bundles strictly reconcile identity, resolution, quantity and exact cost; compatibility costs feed existing sales/valuation views; generic edit/delete blocked |
| Inventory reversal safety | Phase 2C-B locally validated candidate; unpublished | only append-only quantity/cost reversal implemented; current-version and available-after-sales bounds; no negative quantity; refund alone never removes Inventory; richer product-resolution correction deferred |
...
A complete future recovery point includes schema/version manifest, canonical records, provenance, audit references, protected file manifest/content, counts, hashes, and validation results. Phase 1A covers registered browser sources; Phase 1B can additionally include the bounded canonical export only when it is owner-authorized and valid. Phase 2A raised the registry to 22 sources (18 locally included and four excluded/conditional), Phase 2B1 to 23/19, Phase 2D-A to 24/20, and Phase 2C-A to 25/21. Phase 2C-B keeps those totals and extends the existing Deal Finder source to schema 3 with validated nonsecret Inventory lot/application/event/adjustment metadata. Backup validation requires complete, strictly reconciled item/lot/application/event bundles. Plaintext passwords, OTPs, tokens, sessions, OAuth state/codes/verifiers, provider/Bot/retailer/payment/proxy secrets, raw/protected/provider content, credential-bearing URLs, security links, owner-authority fields, Inventory Handoff Preview, and Inventory Creation Candidate remain prohibited. Coverage must say `PARTIAL` whenever a relevant server source or file byte is absent. Restore Preview remains bounded and zero-write; the mixed `deal-finder.inventory`, new Inventory provenance paths, and all Purchase/Receiving paths are `REQUIRES_MAPPING`, and applying restore is not implemented. Any expanded server export, future restore, or deletion operation requires fresh owner authorization and rate limiting.
...
| Delivery, refund, Receiving, preview, or candidate silently mutates Inventory | Phase 2C-B exposes one gateway only behind explicit OWNER confirmation and fresh authoritative derivation | retain no automatic writer/import; test caller authority rejection, eligibility/product/cost recomputation, and explicit action copy |
...
| Generic edit/delete or later return erases acquisition provenance | managed items/lots are protected; creation/application/events remain and quantity/cost reversals append adjustments | retain protected-record UI/repository guards, available-after-sales checks and no physical deletion; defer richer product-resolution correction to Phase 2C-C |
