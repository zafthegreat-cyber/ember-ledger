...
Published Phase 2C-B baseline: `bcff80042a15a29492ed32ba945291b50d35b5bb`. Phase 2C-C is the current local-only Inventory correction/disposition candidate. It adds no automatic evidence import, refund-to-return inference, automatic replacement/extra Inventory, remote persistence, executed schema, migrated owner record, connected email/Inbox/Orders/Bot provider, live Bot control, checkout, billing, AI/computer-vision provider, or autonomous retailer/marketplace action. The separate Phase 2B2-B.1 operational verification remains paused; `hostedRuntimeVerified=false`.
...
| Owner-confirmed local Inventory creation | `/business/purchases` and canonical Business Inventory | CLIENT_LOCAL_ONLY | Inventory creation gateway plus existing Flip Scout repository | schema-4 `inventory`, `inventoryLots`, applications/events/activity | IC + FS | Explicit verified-OWNER mutation only; exact costs feed existing sales/COGS/summary/valuation projections; Web Lock/IDs/read-back provide local idempotency/repair, not server durability |
| Inventory acquisition provenance and reversal | Business Inventory / Purchase history | CLIENT_LOCAL_ONLY | provenance-managed item/lot/event/typed-adjustment records | schema-4 Flip Scout source | IC + ICC + FS | Separate exact-cost lot; generic edit/delete/import blocked; typed append-only chain retains original creation evidence |
| Inventory correction and physical-return disposition | `/business/purchases` | CLIENT_LOCAL_ONLY | `src/features/purchaseReceiving/inventoryCorrection` | ephemeral preview plus schema-4 typed adjustment on confirmation | ICC | Explicit OWNER preview/confirm; product/condition correction is whole-lot and blocked after sale/transfer; returns cannot exceed unsold/untransferred availability; refund != return; replacement/new extra remain separate workflows; Raw/Graded deferred |
| Purchase/Receiving and Inventory backup/preview | Owner Center Data & Backup | PARTIALLY_IMPLEMENTED | backup registry/validator/Restore Preview | sanitized Purchase/Receiving plus schema-4 Deal Finder sections | PR + IC + ICC + backup/preview | Strict complete item/lot/application/event/adjustment-chain reconciliation; mixed `inventory` and provenance paths `REQUIRES_MAPPING`; preview/candidate/private-journal/raw evidence excluded; restore apply absent |
