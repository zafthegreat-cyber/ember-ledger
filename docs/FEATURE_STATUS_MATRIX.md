...
| Mailbox provider trusted runtime | `/api/account-ops/provider-connections` | PARTIALLY_IMPLEMENTED | `api/account-ops/provider-connections.ts`, `backend/src/providerRuntime`, `backend/src/routes/providerConnections.routes.ts` | unavailable production adapters; test-only memory adapters | IO provider runtime + Preview runtime + owner security | Exact Preview function and strict server proof exist; hosted candidate verification is required, while connect/callback/network adapter, durable secret/connection/state store, and live scope remain absent |
