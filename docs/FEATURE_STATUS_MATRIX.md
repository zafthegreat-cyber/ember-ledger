...
| Provider Connections foundation | `/account-ops/connections` | PARTIALLY_IMPLEMENTED | `src/features/accountOps/InboxOrderFoundation.jsx`, `src/services/accountOpsProviderApi.js`, `backend/src/providerRuntime`, `backend/src/routes/providerConnections.routes.ts` | safe server capability/status projection only | IO, AO, owner security | Runtime defaults unavailable; Gmail/Outlook definitions have no active capability, connection, callback, token store, or verified hosted route |
...
| Mailbox provider trusted runtime | `/api/account-ops/provider-connections` | PARTIALLY_IMPLEMENTED | `backend/src/providerRuntime`, `backend/src/routes/providerConnections.routes.ts` | unavailable production adapters; test-only memory adapters | IO provider runtime + owner security | OWNER-only status/capabilities and disconnect contract exist; no connect/callback/network adapter, durable secret/connection/state store, live scope, or verified Preview routing |
| Mailbox secret storage | server-only provider boundary | BACKEND_REQUIRED | `backend/src/providerRuntime/secretStore.ts` | unavailable production adapter | IO security | Test-only injected memory adapter proves the interface; no managed durable server secret store is configured, and browser/backup storage is prohibited |
| OAuth state/replay protection | future provider authorization | BACKEND_REQUIRED | `backend/src/providerRuntime/oauthStateStore.ts` | unavailable production adapter | IO security | Test-only adapter proves bounded owner/provider/redirect binding, expiry and single-use consumption; durable atomic cross-instance store and callback remain absent |
...
The strongest complete features are the minimal shell, manual Deal Inbox/analysis foundations, eBay Browse connector and review gate, core calculations/allocation/quantity validation, purpose history, Phase 1C deterministic local intelligence services, published Phase 2A Account Ops metadata workflows, and focused compatibility/accessibility behavior. Published Phase 2A.5 adds product-workspace navigation and honest homes without claiming that every conceptual Collect/Find/Sell/Bot/Business capability is complete. Local Phase 2B1 adds deterministic minimized message/Order Candidate services and an owner-protected default-unavailable provider runtime, not a connected mailbox. The largest false-positive risk is mistaking a workspace shell or local rule/evidence/alias/provider adapter for AI, licensed market coverage, receiving email, retailer/Bot integration, billing, or durable canonical capability. This matrix therefore keeps Account Ops and Inbox/Order records, card-analysis history, and condition/restock decisions `CLIENT_LOCAL_ONLY`, classifies Bot and entitlement shells conservatively, and keeps live provider ingestion, Purchase import, provider provisioning, sold comparables, and AI appropriately blocked or future.
