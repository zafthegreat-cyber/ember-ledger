...
Published Phase 2C-A / Phase 2C-B starting baseline: `3b10644cf1be9498c08b876b5a3bbef98a24ee1c`. Phase 2C-B is the current local-only owner-confirmed Inventory-creation candidate. It adds no automatic evidence import, automatic Receiving-to-Inventory, remote persistence, executed schema, migrated owner record, connected email/Inbox/Orders/Bot provider, live Bot control, checkout, billing, AI/computer-vision provider, or autonomous retailer/marketplace action. The separate Phase 2B2-B.1 operational verification remains paused; `hostedRuntimeVerified=false`.
...
Test abbreviations: **FS** `test:flip-scout`; **OC** `test:owner-center`; **EB** `test:flip-scout-ebay`; **INT** Phase 1C intelligence/history/provider tests; **AO** published Phase 2A Account Ops domain/fixture/browser tests; **WS** Phase 2A.5 workspace registry/preference/browser tests; **IO** Phase 2B1 provider runtime/inbox/order tests; **BO** Phase 2D-A Bot Operations domain/fixture/security/UI/browser tests; **STP** Phase 2D-B2 Stellar preview parser/security/fixture/zero-write/browser tests; **PR** published Phase 2C-A Purchase/Receiving tests; **IC** locally validated Phase 2C-B Inventory creation candidate/cost/idempotency/security/UI/browser tests; **BR** `test:flip-scout-browser`; **RL** `test:route-loading`; **LR** `test:legacy-routes`; **PL** `test:plain-language`; **A11Y** keyboard/viewport checks; **REG** bounded 28-scenario regression. “None focused” means broader loading/regression coverage may exist but no domain assertion was found.
...
| Owner-confirmed local Inventory creation | `/business/purchases` and canonical Business Inventory | CLIENT_LOCAL_ONLY | Inventory creation gateway plus existing Flip Scout repository | schema-3 `inventory`, `inventoryLots`, applications/events/activity | IC + FS | Explicit verified-OWNER mutation only; exact costs feed existing sales/COGS/summary/valuation projections; Web Lock/IDs/read-back provide local idempotency/repair, not server durability |
| Inventory acquisition provenance and reversal | Business Inventory / Purchase history | CLIENT_LOCAL_ONLY | provenance-managed item/lot/event/adjustment records | schema-3 Flip Scout source | IC + FS | Separate exact-cost lot; generic edit/delete blocked; only append-only quantity/cost reversal exists; refund does not remove Inventory; product-resolution correction deferred to Phase 2C-C |
| Purchase/Receiving and Inventory backup/preview | Owner Center Data & Backup | PARTIALLY_IMPLEMENTED | backup registry/validator/Restore Preview | sanitized Purchase/Receiving plus schema-3 Deal Finder sections | PR + IC + backup/preview | Strict complete item/lot/application/event reconciliation; mixed `inventory` and new provenance paths `REQUIRES_MAPPING`; preview/candidate/raw evidence excluded; restore apply absent |
