...
|---|---|---|---|---|---|---|
| Brand configuration | all | PARTIALLY_IMPLEMENTED | `src/config/brand.js`, brand asset plugin | bundled config | PL, build | Code 3 is approved in documentation but not yet applied to runtime; business name/tagline remain separate; missing social/currency/time-zone/PWA/app-icon fields; next runtime task |
...
| eBay active-listing search | `/find/ebay`, `/find/ebay-search` | IMPLEMENTED | eBay screen/client + backend service | official eBay API via server | EB, RL, REG | Add backend owner guard Phase 1A |
...
| eBay server OWNER authorization | `/api/ebay/health`, `/api/ebay/search` | BACKEND_REQUIRED | no general backend owner middleware | none | Missing auth tests | Exact next task Phase 1A |
...
| JSON backup/import | Sources/Data | IMPLEMENTED | feature storage export/import | local feature document | FS | Not complete whole-app verified backup; Phase 1A |
...
| Page / feature | Route | Status | Current component/module | Storage/data source | Tests | Gap, dependency, recommended phase |
|---|---|---|---|---|---|---|
| Owner Center authorization UI | `/owner-center/*` | PARTIALLY_IMPLEMENTED | `ownerAuthorization.js`, `src/App.jsx` | client profile/local beta identity | OC, RL, REG | Backend enforcement Phase 1A |
...
| Controls / Data and Backup | `/owner-center/controls/data-backup` | PARTIALLY_IMPLEMENTED | feature export/import and legacy backup | mixed local data | FS/legacy backup tests | Complete verified backup/restore; exact next Phase 1A |
...
| Controls / Security | `/owner-center/controls/security` | FRONTEND_ONLY | owner/settings concepts | client profile | OC partial | Sessions/devices/backend auth Phase 1A |
...
| Page / feature | Route | Status | Current component/module | Storage/data source | Tests | Gap, dependency, recommended phase |
|---|---|---|---|---|---|---|
| Owner authentication | auth/onboarding routes | IMPLEMENTED_DIFFERENTLY | legacy client/Supabase auth | Supabase session/profile | auth/REG | Private server session and recovery Phase 1A |
...
| Complete backup and restore | Owner Controls | BACKEND_REQUIRED | partial feature exports | mixed local/legacy | partial tests | Phase 1A contract then Phase 1B files/DB |
