...
Phase 2C-A is published at `3b10644cf1be9498c08b876b5a3bbef98a24ee1c`, and Phase 2C-B at `bcff80042a15a29492ed32ba945291b50d35b5bb`. Phase 2C-C is the current local-only candidate: it retains the explicit verified-OWNER Inventory boundary and adds preview-first, typed append-only correction and physical-return dispositions. It does not connect Inbox/Order or Bot evidence, infer a return from a refund, create replacement/extra Inventory automatically, resume Phase 2B2-B.1, activate remote persistence, or deploy Production.
...
Backup validation requires complete, strictly reconciled application/event/item/lot/typed-adjustment chains. The mixed `deal-finder.inventory` migration path and all provenance paths remain `REQUIRES_MAPPING`. See [INVENTORY_CREATION_CONTRACT.md](./INVENTORY_CREATION_CONTRACT.md) and [INVENTORY_CORRECTION_DISPOSITION_CONTRACT.md](./INVENTORY_CORRECTION_DISPOSITION_CONTRACT.md). Phase 2C-C remains browser-local; the Web Lock and private journal are not a server transaction or a claim of multi-device safety.
...
| Purchase, Receiving, Inventory creation, and correction | OWNER-gated Purchase Draft/Purchase/Receiving, derived handoff/candidate review, explicit Inventory confirmation, and preview-first append-only correction/disposition into the existing local Business Inventory | `src/features/purchaseReceiving`, `src/features/flipScout`, `src/pages/EverydayWorkspaces.jsx` | `LOCAL_ONLY`; refund is not return; replacements/extras use separate Receiving/acquisition review; no upstream auto-import, automatic product creation, remote adapter, or canonical-schema use |
...
- `src/features/flipScout/storageRepository.js` stores schema-versioned feature data under `ember-and-tide.flip-scout.v1`. The old namespace is intentionally retained for compatibility. Phase 2C-C advances this document to schema 4 under the same key and validates provenance-managed Inventory acquisition lots, applications, creation events, and typed append-only adjustments without creating a second Inventory authority.
...
- Phase 2C-A stores validated Purchase Drafts, confirmed Purchases, Purchase events, Receiving Events, and bounded activity under `code3.purchase-receiving.v1`. Inventory Handoff, creation, and correction previews/candidates are derived in memory and are not stored collections. The Inventory gateways may write only after explicit owner confirmation to the existing schema-4 Flip Scout Inventory document; the private write-ahead journal is not application data or a backup source.
