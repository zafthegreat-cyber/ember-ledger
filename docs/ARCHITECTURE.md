...
Phase 2C-A is published at `3b10644cf1be9498c08b876b5a3bbef98a24ee1c`, Phase 2C-B at `bcff80042a15a29492ed32ba945291b50d35b5bb`, Phase 2C-C at `ef30033a3b30989737878252fb31354aaecf68a3`, and Phase 2C-D at `5eef5ae59e79dccb7cbb341f42ca6bda7468a762`. Phase 2C-E is the current local-only, read-only candidate: it derives OWNER-only accountant-review and prior-period projections from immutable Sales and append-only reconciliation history. It does not post accounting entries, infer tax treatment or filing status, mutate any business record, add an export or note writer, begin managed Transfer or Raw/Graded authority, resume Phase 2B2-B.1, activate remote persistence, or deploy Production.
...
## Phase 2C-B through Phase 2C-E Inventory and review delta

Phase 2C-B reuses, rather than replaces, the writable Business Inventory under `ember-and-tide.flip-scout.v1`. It added acquisition `inventoryLots`, deterministic `inventoryCreationApplications`, append-only `inventoryCreationEvents`, and append-only `inventoryAdjustments` beside the existing `inventory` collection. Phase 2C-C retains the same key and advances the document to schema version 4, where `inventoryAdjustments` is a typed append-only correction/disposition/reversal history. Phase 2C-D advances the same document to schema version 5 by adding `inventoryReconciliationEvents`; it does not introduce a competing Inventory or accounting source.

- `InventoryCreationCandidate` is an ephemeral projection from one confirmed Purchase, current owner-confirmed Receiving, reviewed product identity/condition/disposition, and exact acquisition allocation. It is not persisted or backed up.
- Only received quantity remains eligible. Purchase `RETURN_INITIATED`/`RETURNED`/`CANCELLED` and Receiving `RETURNED_TO_SENDER`/`CANCELLED`/`MISSING`/`NOT_RECEIVED` block candidates. Ambiguous/unresolved product identity, unknown condition, unresolved extra/wrong/substituted items, and unsafe damaged-item disposition also block confirmation. Manual owner resolution must reference an existing local Inventory/product relationship and preserves its bounded reason across provenance records.
- The verified OWNER gate precedes every Inventory read/write. Confirmation re-derives candidate identity, version, eligible quantity, product resolution, and cost inside a same-origin exclusive Web Lock.
- Deterministic IDs, one normalized Flip Scout document write, and read-back verification provide same-origin retry/idempotency and compatible interrupted-write repair. Conflicting semantics or stale versions fail closed.
- Each acquisition remains a separate provenance-managed Inventory item and lot, even for an existing product; no cost averaging destroys lot provenance.
- Exact unit costs use integer minor units and deterministic remainder distribution. Partial receipts consume disjoint cost slices in authoritative append order, so mutable timestamps or client-chosen IDs cannot reassign earlier pennies; their compatibility projections feed existing Business/Flip Scout sales, COGS, summary, and valuation UI.
- Phase 2C-C adds explicit owner preview and confirmation for supported whole-lot product/condition corrections, bounded physical return/quantity dispositions, pre-sale/pre-transfer cost correction, and eligible reversal repair. Refunds do not remove Inventory, product/condition correction and cost correction are blocked after sales/transfers, and physical return is limited to unsold/untransferred availability.
- Replacement items require an effective unreversed owner-confirmed return, a scoped and idempotently resumable `REPLACEMENT_NOTED` link, a new physical Receiving Event, and a separate Inventory-creation review. The completed provenance bundle retains both dependency references and blocks later reversal of the consumed return; the new lot reuses the exact returned unit-cost slice, so generic over-receipt and duplicate stock/cost are blocked. Unexpected extras require a separate acquisition and cost review. Neither mutates an existing lot automatically. Raw card and Graded card remain deferred to a type-specific card/slab condition workflow.
- Schema-4 provenance-managed Inventory has no transfer collection or writer. Runtime correction explicitly proves that invariant and fails closed if transfer authority/evidence appears; the legacy Collection-to-Forge movement belongs to a different state authority and is not a managed Inventory transfer.
- Phase 2C-D adds an ephemeral `InventoryReconciliationCandidate` only where a correction affects immutable downstream Sales or a future managed Transfer. Completed Sale rows retain original product, allocation sequence, exact COGS, profit, and ROI; confirmed events record exact signed deltas and derived effective projections instead of rewriting those bytes.
- For partial sales, corrected unit cost uses the original deterministic allocation order. Sold-unit differences become realized COGS deltas, the unsold suffix becomes current Inventory-cost change, and both sides reconcile exactly to the total lot-cost effect. Negative COGS deltas are valid signed adjustments; canonical acquisition costs remain non-negative integer minor units.
- No canonical managed-transfer collection exists in this phase. Transfer categories are represented for honest review but remain `NEEDS_REVIEW`/blocked. Legacy movement is not promoted into transfer authority and multi-hop results are never guessed.
- Phase 2C-D reporting may show original historical facts plus an explicit reconciled projection. It may not silently substitute current Inventory product/cost values for the immutable Sale-time values. Prior-period source date and reconciliation date remain separate.
- Phase 2C-E adds only an ephemeral `AccountantReviewItem` projection over validated local history. Calendar month/quarter/year classification treats an exact Sale `YYYY-MM-DD` as recorded and derives an ISO reconciliation instant's date in UTC. The UI keeps `ORIGINAL_RECORDED` and `CURRENT_EFFECTIVE` exact-minor-unit values separate, uses `FILING_STATUS_UNKNOWN`, and gives cautious review guidance rather than tax conclusions.
- Accountant Review has no repository, ledger, journal entry, filing action, export, note, or mutation method. Reversal rows remain visible and signed deltas are counted exactly once. Refund and physical return remain separate, while Transfer review remains blocked without canonical managed-Transfer authority.
- Generic edit/delete/import paths cannot alter provenance-managed items/lots. The private crash-recovery journal is operational state, not a backup or migration source.

Backup validation requires complete, strictly reconciled application/event/item/lot/typed-adjustment/reconciliation chains. The mixed `deal-finder.inventory`, Sale, and provenance paths remain `REQUIRES_MAPPING`. Accountant Review is regenerated and adds no backup or migration source. See [INVENTORY_CREATION_CONTRACT.md](./INVENTORY_CREATION_CONTRACT.md), [INVENTORY_CORRECTION_DISPOSITION_CONTRACT.md](./INVENTORY_CORRECTION_DISPOSITION_CONTRACT.md), [INVENTORY_RECONCILIATION_CONTRACT.md](./INVENTORY_RECONCILIATION_CONTRACT.md), and [ACCOUNTANT_REVIEW_CONTRACT.md](./ACCOUNTANT_REVIEW_CONTRACT.md). Phase 2C-E remains browser-local; the Web Lock and private journal are not a server transaction or a claim of multi-device safety.
...
| Purchase, Receiving, Inventory creation/correction/reconciliation, and Accountant Review | OWNER-gated Purchase Draft/Purchase/Receiving, explicit Inventory confirmations, append-only correction/reconciliation history, and read-only original-versus-effective period review | `src/features/purchaseReceiving`, `src/features/flipScout`, `src/pages/EverydayWorkspaces.jsx` | `LOCAL_ONLY`; Accountant Review is ephemeral and cannot post, export, note, file, or mutate; original Sales/Transfers remain immutable; transfer modes remain blocked without managed authority |
...
- `src/features/flipScout/storageRepository.js` stores schema-versioned feature data under `ember-and-tide.flip-scout.v1`. The old namespace is intentionally retained for compatibility. Phase 2C-D advances this document to schema 5 under the same key and validates provenance-managed Inventory acquisition lots, applications, creation events, typed append-only adjustments, immutable managed Sales, and append-only reconciliation events without creating a second Inventory or accounting authority.
...
- Phase 2C-A stores validated Purchase Drafts, confirmed Purchases, Purchase events, Receiving Events, and bounded activity under `code3.purchase-receiving.v1`. Inventory Handoff, creation, correction, and reconciliation previews/candidates are derived in memory and are not stored collections. The Inventory gateways may write only after explicit owner confirmation to the existing schema-5 Flip Scout Inventory document; the private write-ahead journal is not application data or a backup source.
- Phase 2C-E Accountant Review items, filters, period groups, and summaries are pure derived memory state over those existing documents. They add no localStorage key, collection, migration path, backup source, remote adapter, or accounting mutation authority.
...
Published Phase 2A adds passing profile, alias/template/collision, password, retailer-account/setup/health/task, recursive authority rejection, backup/preview, route, mobile, and deterministic fixture tests. Published Phase 2A.5 adds registry, bounded-preference, switcher, deep-link, authority, legacy-route, mobile, shared-record projection, and inherited regression evidence. Published Phase 2B1 focused validation passes 16/16 provider-runtime cases, 52 domain assertions, 25/25 deterministic fixtures with 56 assertions, 102 history/idempotency assertions, and 55 security/protected-message assertions. Backup validation reports 19 included sections and 18 fixture records; Restore Preview and migration checks remain zero-write. The inherited gate also passes frontend/backend builds and the bounded 28/28 regression in 444.527 seconds with zero retries and no open handles. Phase 2D-A adds focused provider-registry, adapter, domain, fixture, security, idempotency/reconciliation, backup, Restore Preview, OWNER-gate, UI, and responsive-browser coverage. Published Phase 2C-A adds focused Purchase/Receiving coverage. Published Phase 2C-B passed 48 candidate, 18 exact-cost, 67 idempotency/journal, 65 security, 39/39 fixture with 197, and 65 UI assertions; four dedicated browser scenarios, both 46-view theme matrices, frontend/backend builds, zero-write backup/preview/migration gates, and the complete 28/28 regression also passed. Published Phase 2C-C/2C-D evidence is recorded in [IMPLEMENTATION_STATUS.md](./IMPLEMENTATION_STATUS.md); Phase 2C-E results must be reported only after its local gate completes. No test evidence connects a mailbox or Bot provider, activates remote Inventory, or posts accounting/tax data.
