...
Phase 2C-A is published at `3b10644cf1be9498c08b876b5a3bbef98a24ee1c`. Phase 2C-B is the current local-only extension: it adds an ephemeral Inventory Creation Candidate and one explicit verified-OWNER confirmation boundary into the existing local Business Inventory. It does not connect Inbox/Order or Bot evidence, create a Purchase or Inventory automatically, create products from titles, resume Phase 2B2-B.1, activate remote persistence, or deploy Production.
...
## Phase 2C-B owner-confirmed Inventory delta

Phase 2C-B reuses, rather than replaces, the writable Business Inventory under `ember-and-tide.flip-scout.v1`. Flip Scout schema version 3 adds acquisition `inventoryLots`, deterministic `inventoryCreationApplications`, append-only `inventoryCreationEvents`, and append-only `inventoryAdjustments` beside the existing `inventory` collection.
...
- Phase 2C-B implements append-only quantity/cost reversal only. Refunds do not remove Inventory, sold/unavailable quantity cannot be reversed, and original creation/application/cost provenance is retained. Richer post-creation product-resolution correction is deferred to Phase 2C-C.
- Generic edit/delete paths cannot alter Phase 2C-B provenance-managed items/lots.

Backup validation requires complete, strictly reconciled application/event/item/lot bundles. The mixed `deal-finder.inventory` migration path and all new provenance paths remain `REQUIRES_MAPPING`. See [INVENTORY_CREATION_CONTRACT.md](./INVENTORY_CREATION_CONTRACT.md). Phase 2C-B remains browser-local; the Web Lock is not a server transaction or a claim of multi-device safety.
...
| Purchase, Receiving, and Inventory creation | OWNER-gated Purchase Draft/Purchase/Receiving, derived handoff/candidate review, and explicit Inventory confirmation into the existing local Business Inventory | `src/features/purchaseReceiving`, `src/features/flipScout`, `src/pages/EverydayWorkspaces.jsx` | `LOCAL_ONLY`; no upstream auto-import, receipt inference, automatic Inventory/product creation, remote adapter, or canonical-schema use |
...
- `src/features/flipScout/storageRepository.js` stores schema-versioned feature data under `ember-and-tide.flip-scout.v1`. The old namespace is intentionally retained for compatibility. Phase 2C-B normalizes this document to schema 3 and adds provenance-managed Inventory acquisition lots, applications, creation events, and adjustments without creating a second Inventory authority.
...
- Phase 2C-A stores validated Purchase Drafts, confirmed Purchases, Purchase events, Receiving Events, and bounded activity under `code3.purchase-receiving.v1`. Inventory Handoff Preview and Phase 2C-B Inventory Creation Candidate are derived in memory and are not stored collections. The separate Phase 2C-B gateway may write only after explicit owner confirmation to the existing schema-3 Flip Scout Inventory document.
