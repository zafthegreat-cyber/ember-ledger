...
Phase 2C-D is published through `5eef5ae59e79dccb7cbb341f42ca6bda7468a762`; Phase 2C-E is the parallel local-only, read-only Accountant Review and prior-period projection candidate. No mailbox, OAuth app, Bot, retailer account, proxy, credential provider, billing provider, checkout provider, accountant integration, or tax service is connected. The separate Phase 2B2-B.1 operational verification remains paused; a Free Upstash resource exists but the remaining owner/CORS/activation configuration is incomplete and `hostedRuntimeVerified=false`.
...
- Shipment or delivery evidence is not Receiving. A Receiving Event and Inventory Handoff Preview do not themselves create Inventory. Phase 2C-B requires a fresh ephemeral candidate, exact product/cost/quantity review, and separate explicit verified-OWNER confirmation.
- Inventory Creation Candidate is not Inventory. Same-origin Web Locks and deterministic local application IDs are not a remote transaction or provider integration.
- An Inventory correction/disposition preview is not a mutation. A refund is not a physical return; replacement requires new Receiving, and an unexpected extra requires separate acquisition/cost review.
- An Inventory reconciliation preview is not an historical mutation. Original Sale/Transfer facts remain immutable; signed COGS/product projections are append-only review results, not a provider feed or remote accounting ledger.
- Accountant Review is a derived local projection, not an accountant connection, bookkeeping export, posted adjustment, filing status, tax conclusion, or permission to mutate a Sale, Purchase, Inventory, or Transfer record.
...
| Receiving and Inventory Handoff Preview | AVAILABLE_LOCAL_ONLY | append-only local Receiving Events and derived preview | synthetic/local owner-confirmed receiving only | no delivery inference or automatic Inventory mutation; preview/candidate stay non-authoritative | Published Phase 2C-A |
| Owner-confirmed local Inventory creation | AVAILABLE_LOCAL_ONLY | published Phase 2C-B candidate/gateway into existing Flip Scout Business Inventory | explicit verified-OWNER confirmation of synthetic/local confirmed Receiving only | return/cancel/missing states block; manual product resolution requires an existing local relationship/reason; no server transaction, automatic product/upstream import, or provider input | Published Phase 2C-B |
| Inventory acquisition lot/provenance | AVAILABLE_LOCAL_ONLY | separate exact-cost item/lot plus append-only application/event/typed adjustment/reconciliation | owner-confirmed local acquisition plus reviewed correction/disposition/reversal/reconciliation metadata; exact slices feed existing sales/COGS/summary/valuation views | generic edit/delete/import blocked; mixed Inventory/provenance paths require mapping | Phase 2C-B–D local foundation |
| Inventory correction and physical-return disposition | AVAILABLE_LOCAL_ONLY | published Phase 2C-C preview/confirmation gateway into schema-5 local Inventory | owner reviews whole-lot product/condition correction, bounded return/quantity disposition, pre-sale/pre-transfer cost correction, or eligible reversal repair | refund does not return stock; replacement uses new Receiving; extras use separate acquisition; Raw/Graded deferred; no remote transaction | Published Phase 2C-C |
| Historical COGS/Sale reconciliation | AVAILABLE_LOCAL_ONLY | published Phase 2C-D reconciliation gateway and derived reporting projections | explicit owner review of exact signed COGS/product/cost effect while original Sale remains immutable | no broad accounting ledger; original Sale dates and reconciliation timestamps remain distinct; server transaction/multi-device authority remains future | Published Phase 2C-D |
| Accountant Review / prior-period projection | AVAILABLE_LOCAL_ONLY | Phase 2C-E pure derived projection in the OWNER-gated Business Purchases workflow | validated local Sale/lot/Purchase/Receiving/reconciliation history only | read-only; UTC correction-date basis; `FILING_STATUS_UNKNOWN`; no accountant/tax connection, ledger/journal entry, filing/amendment, export, note, backup, migration source, or mutation | Phase 2C-E local candidate |
| Managed Transfer reconciliation | NOT_CONFIGURED | bounded Phase 2C-D category/blocker vocabulary only | review/blocked state only | no canonical managed-transfer collection or source→destination transaction authority; multi-hop cannot be confirmed safely | Future separately reviewed transfer authority |
