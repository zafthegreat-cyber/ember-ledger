...
| Inventory acquisition lot/provenance | AVAILABLE_LOCAL_ONLY | separate exact-cost item/lot plus append-only application/event/typed adjustment | owner-confirmed local acquisition plus reviewed correction/disposition/reversal metadata; exact slices feed existing sales/COGS/summary/valuation views | generic edit/delete/import blocked; mixed Inventory/provenance paths require mapping | Phase 2C-B foundation + Phase 2C-C candidate |
| Inventory correction and physical-return disposition | AVAILABLE_LOCAL_ONLY | Phase 2C-C preview/confirmation gateway into schema-4 local Inventory | owner reviews whole-lot product/condition correction, bounded return/quantity disposition, pre-sale/pre-transfer cost correction, or eligible reversal repair | refund does not return stock; sales/transfers block identity/condition/cost rewrite; replacement uses new Receiving; extras use separate acquisition; Raw/Graded deferred; no remote transaction | Phase 2C-C local candidate |
