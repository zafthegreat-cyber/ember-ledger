...
| Owner-confirmed local Inventory creation | AVAILABLE_LOCAL_ONLY | Phase 2C-B candidate/gateway into existing Flip Scout Business Inventory schema 3 | explicit verified-OWNER confirmation of synthetic/local confirmed Receiving only | return/cancel/missing states block; manual product resolution requires an existing local relationship/reason; no server transaction, automatic product/upstream import, or provider input | Phase 2C-B locally validated; publication pending |
| Inventory acquisition lot/provenance | AVAILABLE_LOCAL_ONLY | separate exact-cost item/lot plus append-only application/event/adjustment | owner-confirmed local acquisition and quantity/cost reversal metadata; exact slices feed existing sales/COGS/summary/valuation views | generic edit/delete blocked; mixed Inventory/new paths require mapping; product-resolution correction and broader dispositions need Phase 2C-C review | Phase 2C-B foundation |
