...
| Bot provider-neutral local foundation | AVAILABLE_LOCAL_ONLY | `src/features/botOps` registry/contracts, local metadata/evidence service, security/reconciliation, test-only mock adapter, and static discovery evidence | owner-reviewed local metadata; synthetic test input; official-source research references | no SDK/network/bridge/webhook/export watcher, credential store, live health or task command | Phase 2D-A foundation + Phase 2D-B1 discovery |
...
| Stellar Bot provider | NOT_CONFIGURED | static safe registry plus official-source capability review; all live capabilities false | none | documented manual task export has unverified exact format/version rules; Discord is notification-only; documented WebSocket is input-to-Stellar rather than status egress | Offline sanitized task-export design candidate only; no live pilot |
...
The review found no verified read-only task/status/history API for Hayha or Stellar. Hayha's public material documents human GUI/CLI use and limited Discord notifications, while its published terms prohibit automated access, data extraction, and reverse engineering. Stellar documents three materially different mechanisms: Discord notifications; manual task export/import controls whose exact serialized format/version rules are unverified by the current public overview; and an external product-monitor WebSocket that feeds pings into Stellar. None is a live Code 3 read/status adapter. Profile, account/session, proxy, and full-config exports may contain credentials, payment data, or other sensitive material and are not eligible inputs.

The current recommendation is `NO_LIVE_BOT_PILOT_YET`. The least-risk future precursor is a dry-run parser for a synthetic or provider-approved sanitized Stellar task export selected explicitly by the owner after current format/version confirmation. It must reject profiles, accounts, sessions, proxies, config, license values, credential-bearing URLs, and unknown secret fields, perform no write back to Stellar, and remain separate from task control. See [BOT_PROVIDER_CAPABILITY_REVIEW.md](./BOT_PROVIDER_CAPABILITY_REVIEW.md).
