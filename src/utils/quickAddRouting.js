...
export function calendarEventToQuickAddSeed(event = {}, destinations = { vault: true }) {
  ...
  return {
    ...
    marketValueSource: event.marketValueSource || event.market_value_source || event.marketSource || event.market_source || event.sourceLabel || event.source || "",
    marketPriceConfidence: event.marketPriceConfidence || event.market_price_confidence || event.priceConfidence || event.price_confidence || "",
    msrpPrice: event.msrpPrice || event.msrp || "",
    msrpSource: event.msrpSource || event.msrp_source || "",
    notes,
