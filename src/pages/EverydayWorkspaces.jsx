...
function acquisitionCost(record = {}) {
...
function useBusinessRepository() {
  ...
  const saveRecord = useCallback((collection, record, message = "Record saved") => {
    const result = repository.upsert(collection, record);
    const activityResult = repository.save({ ...result.state, activity: [activityEntry(message, recordTitle(result.record)), ...result.state.activity].slice(0, 150) });
    setState(activityResult.state);
    setError(result.error || activityResult.error);
    return result.record;
  }, [repository]);

  const removeRecord = useCallback((collection, record) => {
    if (!window.confirm(`Delete ${recordTitle(record)}? This cannot be undone.`)) return false;
    const result = repository.remove(collection, record.id);
    setState(result.state);
    setError(result.error);
    return true;
  }, [repository]);

  const allocateLot = useCallback((lot, method) => {
    ...
    const result = repository.save({ ...state, lots: state.lots.map((row) => row.id === lot.id ? { ...row, allocationMethod: method, updatedAt: new Date().toISOString() } : row), inventory: state.inventory.map((item) => byId.get(item.id) || item), activity: [activityEntry("Lot cost allocated", `${recordTitle(lot)} · ${method.replaceAll("_", " ")}`), ...state.activity].slice(0, 150) });
    setState(result.state);
    setError(result.error);
  }, [repository, state]);
...
function RecordForm({ type, seed = {}, state, onSave, onCancel }) {
  ...
  const suggestedCogs = selectedInventory && number(selectedInventory.quantity) > 0 ? number(selectedInventory.allocatedItemCost || selectedInventory.totalPurchaseCost) / number(selectedInventory.quantity) * number(form.quantitySold) : 0;
  const saleResult = calculateSaleResults({ ...form, allocatedCostOfGoodsSold: form.allocatedCostOfGoodsSold === "" ? suggestedCogs : form.allocatedCostOfGoodsSold });
  ...
  const submit = () => {
    ...
    const record = type === "sale" ? { ...form, allocatedCostOfGoodsSold: form.allocatedCostOfGoodsSold === "" ? suggestedCogs : form.allocatedCostOfGoodsSold, netProceeds: saleResult.netProceeds, realizedProfit: saleResult.realizedProfit, realizedRoi: saleResult.realizedRoi } : form;
    onSave(config.collection, record, `${config.title} saved`);
    clearDraft();
    onCancel();
  };

  return <GuidedFormPage title={config.title} description={config.description} steps={config.steps} activeStep={step} onStepChange={setStep} onCancel={cancel} onSubmit={submit} canContinue={canContinue} submitLabel={seed.id ? `Update ${config.submit.replace("Save ", "")}` : config.submit} error={error}>
...
export function CollectionWorkspace({ items = [], initialView = "collection", onViewChange, onAddItem, onChangePurpose, onSellItem, featureControls = EMPTY_FEATURE_CONTROLS }) {
  ...
  const compatibility = useMemo(() => summarizePurposeCompatibility(allItems), [allItems]);
  const visible = useMemo(() => allItems.filter((item) => !query || [recordTitle(item), item.setName, item.cardNumber, item.storageLocation].join(" ").toLowerCase().includes(query.toLowerCase())), [allItems, query]);
  ...
  const tabs = [{ key: "collection", label: "My Collection" }, { key: "sets", label: "Sets & Binders" }, { key: "wishlist", label: "Wishlist" }, featureControls.grading !== false ? { key: "grading", label: "Grading" } : null].filter(Boolean);
  ...
  const changePurpose = (item, nextPurpose) => {
    if (item._recordOrigin === "repository") {
      ...
      saveRecord("inventory", changed, "Item purpose changed");
      setSelected({ ...changed, _recordOrigin: "repository" });
  ...
  const cardsFor = (records, action = null, actionLabel = "") => records.map((item) => <RecordListCard key={item.id} record={item} eyebrow={item.productClassification || "Owned item"} status={item.ownedItemPurpose === OWNED_ITEM_PURPOSES.UNASSIGNED ? "Needs purpose" : item.ownedItemPurpose.replaceAll("_", " ").toLowerCase()} statusTone={item.ownedItemPurpose === OWNED_ITEM_PURPOSES.UNASSIGNED ? "warning" : "neutral"} facts={[{ label: "Acquisition", value: acquisitionCost(item) == null ? "Not recorded" : money(acquisitionCost(item)) }, { label: "Set", value: item.setName || "Not recorded" }, { label: "Condition", value: item.condition || "Not recorded" }, { label: "Location", value: item.storageLocation || "Not assigned" }]} onOpen={setSelected} action={action} actionLabel={actionLabel} />);
  return <main className="everyday-workspace" data-testid="collection-workspace">
    <PageHeader eyebrow="Owned items" title="Collection" description="Personal items, sets, wishlist targets, and grading plans." actions={<PrimaryButton onClick={() => setForm({ type: "collection", seed: {} })}>Add Collection Item</PrimaryButton>} />
    ...
    {unassigned.length ? <section className="everyday-attention"><SectionHeader title="Needs attention" description={`${unassigned.length} owned item${unassigned.length === 1 ? " needs" : "s need"} a purpose before it can be placed reliably.`} actions={<PrimaryButton onClick={() => setSelected(unassigned[0])}>Review Unassigned</PrimaryButton>} /></section> : compatibility.compatibilityCount ? <section className="everyday-attention"><SectionHeader title="Compatibility review" description={`${compatibility.compatibilityCount} existing record${compatibility.compatibilityCount === 1 ? " uses" : "s use"} legacy fields. No irreversible migration has run.`} /></section> : null}
    <MetricRow items={[{ label: "Personal items", value: collectionItems.length }, { label: "Sets & binders", value: sets.length }, { label: "Wishlist", value: heldItems.length }, { label: "Needs purpose", value: unassigned.length, tone: unassigned.length ? "warning" : "neutral" }]} />
    <div className="everyday-toolbar"><FilterButton active={filtersOpen} onClick={() => setFiltersOpen(true)}>Search & Filters</FilterButton></div>
    {view === "collection" ? <section><SectionHeader title="My Collection" description="Recently added personal collection items." />{collectionItems.length ? <div className="everyday-record-list">{cardsFor(collectionItems, changePurpose, "Sell This Item")}</div> : <EmptyState title="No collection items yet" action={<PrimaryButton onClick={() => setForm({ type: "collection", seed: {} })}>Add Collection Item</PrimaryButton>}>Add an item or assign an existing item to Personal collection.</EmptyState>}</section> : null}
    ...
    <BottomSheet open={filtersOpen} title="Collection search" onClose={() => setFiltersOpen(false)}><SearchField label="Search owned items" value={query} onChange={(event) => setQuery(event.target.value)} /><QuietButton onClick={() => { setQuery(""); setFiltersOpen(false); }}>Clear Search</QuietButton></BottomSheet>
  </main>;
}
...
function BusinessDetail({ type, record, state, onBack, onEdit, onDelete, onMoveToCollection, onOpenRelated, onAllocate }) {
  ...
  const summary = purchase ? [{ label: "Purchase cost", value: money(record.totalPurchaseCost), numeric: true }, { label: "Lots", value: linkedLots.length }, { label: "Inventory items", value: linkedInventory.length }, { label: "Date", value: dateLabel(record.purchaseDate) }]
    : lot ? [{ label: "Lot cost", value: money(reconciliation.totalCost), numeric: true }, { label: "Allocated", value: money(reconciliation.allocatedCost), numeric: true }, { label: "Unallocated", value: money(reconciliation.unallocatedCost), numeric: true }, { label: "Items", value: linkedInventory.length }]
      : inventory ? [{ label: "Quantity", value: record.quantity || 0 }, { label: "Allocated cost", value: money(record.allocatedItemCost), numeric: true }, { label: "Projected resale", value: money(record.projectedResaleMid), numeric: true }, { label: "Sales", value: linkedSales.length }]
        : sale ? [{ label: "Gross", value: money(record.grossSalePrice), numeric: true }, { label: "Net proceeds", value: money(record.netProceeds ?? result.netProceeds), numeric: true }, { label: "Realized profit", value: money(record.realizedProfit ?? result.realizedProfit), numeric: true }, { label: "Realized ROI", value: roi(record.realizedRoi ?? result.realizedRoi), numeric: true }]
  ...
  return <RecordDetailPage eyebrow={typeLabel} title={recordTitle(record)} status={lot ? reconciliation.reconciled ? "Reconciled" : "Needs allocation" : record.status || "Recorded"} statusTone={lot && !reconciliation.reconciled ? "warning" : "neutral"} image={recordImage(record)} identity={inventory ? [record.productClassification, record.setName, record.cardNumber].filter(Boolean).join(" · ") : record.source || record.salesChannel || record.merchant || record.purpose || "Business record"} summary={summary} primaryAction={inventory ? <PrimaryButton onClick={() => onMoveToCollection(record)}>Move to Collection</PrimaryButton> : canApplyAllocation ? <PrimaryButton onClick={() => onAllocate(record, record.allocationMethod)}>Apply Allocation</PrimaryButton> : <PrimaryButton onClick={() => onEdit(type, record)}>Edit</PrimaryButton>} secondaryActions={<>{!primaryIsEdit ? <QuietButton onClick={() => onEdit(type, record)}>Edit</QuietButton> : null}{record._recordOrigin !== "legacy" ? <QuietButton onClick={() => onDelete(type, record)}>Delete</QuietButton> : null}</>} sections={[{ title: "Details", description: "Recorded information and assumptions.", children: <DetailList items={details} /> }]} timeline={[record.updatedAt ? { id: "updated", title: "Record updated", date: dateLabel(record.updatedAt) } : null, record.createdAt ? { id: "created", title: "Record created", date: dateLabel(record.createdAt) } : null].filter(Boolean)} related={relatedRows.length ? <div className="related-record-list">{relatedRows.map((row) => <QuietButton key={`${row._type}-${row.id}`} onClick={() => onOpenRelated(row._type, row)}>{recordTitle(row)}</QuietButton>)}</div> : null} onBack={onBack} />;
}

export function BusinessWorkspace({ items = [], purchases = [], sales = [], expenses = [], mileage = [], initialView = "purchases", initialMoneyView = "expenses", onViewChange, onMoneyViewChange, onMoveToCollection }) {
  ...
  const tabs = [{ key: "purchases", label: "Purchases" }, { key: "inventory", label: "Inventory" }, { key: "sales", label: "Sales" }, { key: "money", label: "Money" }];
  const moneyTabs = [{ key: "expenses", label: "Expenses" }, { key: "mileage", label: "Mileage" }, { key: "reports", label: "Reports" }, { key: "reconciliation", label: "Reconciliation" }];
  const changeView = (next) => { setSelected(null); setView(next); onViewChange?.(next); };
  ...
  const editRecord = (type, record) => setForm({ type: type === "purchase" ? "purchase" : type, seed: record });
  const deleteRecord = (type, record) => { const collection = { purchase: "purchases", lot: "lots", inventory: "inventory", sale: "sales", expense: "expenses", mileage: "mileage" }[type]; if (record._recordOrigin === "legacy") return; if (removeRecord(collection, record)) setSelected(null); };
  ...
  const grossSales = allSales.filter((row) => row.status !== "Draft").reduce((sum, row) => sum + number(row.grossSalePrice), 0);
  const realizedProfit = allSales.filter((row) => row.status !== "Draft").reduce((sum, row) => sum + number(row.realizedProfit ?? calculateSaleResults(row).realizedProfit), 0);
  ...
  const missingInventoryCost = allInventory.filter((row) => acquisitionCost(row) == null);
  const draftSales = allSales.filter((row) => row.status === "Draft");
  ...
  return <main className="everyday-workspace" data-testid="business-workspace">
    <PageHeader eyebrow="Business records" title="Business" description="Purchases, resale inventory, sales, and money records." />
