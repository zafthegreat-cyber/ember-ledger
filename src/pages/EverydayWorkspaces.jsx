...
export function CollectionWorkspace({ items = [], initialView = "collection", onViewChange, onAddItem, onChangePurpose, onSellItem, featureControls = EMPTY_FEATURE_CONTROLS }) {
  ...
  return <main className="everyday-workspace" data-testid="collection-workspace">
    ...
    <BottomSheet open={filtersOpen} title="Collection search" onClose={() => setFiltersOpen(false)}><SearchField label="Search owned items" value={query} onChange={setQuery} enterKeyHint="search" autoComplete="off" /><QuietButton onClick={() => { setQuery(""); setFiltersOpen(false); }}>Clear Search</QuietButton></BottomSheet>
  </main>;
}
