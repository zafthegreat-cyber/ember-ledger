...
function StoreProfile({ store, events, visits, onBack }) {
  ...
  return <section data-testid="restock-store-profile"><QuietButton onClick={onBack}>← All Stores</QuietButton><SectionHeader eyebrow={store.retailer} title={store.store || store.name || "Store profile"} description={store.address || "Address not recorded"} /><div className="owner-metric-strip"><EmptyMetric label="Last confirmed restock" value={last ? displayDate(last.eventTime) : null} /><EmptyMetric label="Common weekdays" value={pattern.mostCommonWeekday} /><EmptyMetric label="Common time window" value={pattern.mostCommonTimeWindow} /><EmptyMetric label="Average days between confirmations" value={pattern.averageIntervalDays == null ? null : `${pattern.averageIntervalDays.toFixed(1)} days`} /><MetricCard label="Supporting report count" value={pattern.supportingReportCount} /><MetricCard label="Pattern confidence" value={pattern.patternStability} /><EmptyMetric label="Typical products" value={store.typicalProducts || store.products} /><EmptyMetric label="Typical sellout speed" value={store.typicalSelloutTime} /><MetricCard label="Successful visits" value={storeVisits.filter((visit) => visit.successful).length} /><MetricCard label="Unsuccessful visits" value={storeVisits.filter((visit) => visit.successful === false).length} /></div><RecordCard><h3>Store details</h3><dl className="owner-definition-list"><div><dt>Distance</dt><dd>{asNumber(store.distance) == null ? "Not recorded" : `${store.distance} miles`}</dd></div><div><dt>Stocking model</dt><dd>{store.stockingModel || (store.vendorStocked ? "Vendor-stocked" : store.storeStocked ? "Store-stocked" : "Not recorded")}</dd></div><div><dt>Usual weekday pattern</dt><dd>{store.usualWeekdayPattern || pattern.mostCommonWeekday || "Not enough data"}</dd></div><div><dt>Usual time range</dt><dd>{store.usualTimeRange || pattern.mostCommonTimeWindow || "Not enough data"}</dd></div><div><dt>Notes</dt><dd>{store.notes || "No notes"}</dd></div></dl></RecordCard></section>;
}
