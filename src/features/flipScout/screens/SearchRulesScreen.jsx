...
export default function SearchRulesScreen({ rules, onSave, onDelete, onOpenEbay }) {
  ...
  const save = (event) => {
    ...
    onSave("searchRules", form, { title: form.id ? "Search rule updated" : "Search rule added", detail: `${form.ruleName} · ${form.enabled ? "enabled" : "disabled"}` });
    setForm(blankRule());
    setFormOpen(false);
    ...
    <div className="flip-screen">
      <section className="flip-section">
        ...
        {formOpen ? <form className="flip-form" onSubmit={save}>
          ...
          <FormActions><button type="submit" className="primary-button">{form.id ? "Update rule" : "Save rule"}</button></FormActions>
      ...
      <section className="flip-section">
        ...
        {rules.length ? <div className="flip-record-list">{rules.map((rule) => <article className="flip-record-card" key={rule.id}>
          ...
          <div className="flip-record-actions">{/ebay|any approved source/i.test(rule.marketplace || "") ? <button type="button" className="primary-button" onClick={() => onOpenEbay(rule.id)}>Open in eBay Search</button> : null}<button type="button" className="secondary-button" onClick={() => { setForm({ ...blankRule(), ...rule }); setFormOpen(true); window.scrollTo?.({ top: 0, behavior: "smooth" }); }}>Edit</button><button type="button" className="ghost-button flip-delete-button" onClick={() => onDelete("searchRules", rule.id, rule.ruleName)}>Delete</button></div>
      ...
      <section className="flip-section">
        ...
        <div className="flip-template-grid">{SEARCH_RULE_TEMPLATES.map((template) => <article key={template.id}><span>Inactive template</span><h3>{template.ruleName}</h3><p>{template.notes}</p><button type="button" className="secondary-button" onClick={() => useTemplate(template)}>Use template</button></article>)}</div>
      </section>
    </div>
  );
}
