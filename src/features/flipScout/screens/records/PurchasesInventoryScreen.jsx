...
export default function PurchasesInventoryScreen({ view, state, onSave, onDelete, onAllocateLot }) {
  ...
  if (view === "purchases") return (
    <div className="flip-record-workspace">
      ...
      {inventoryFormOpen ? <InventoryForm form={inventoryForm} set={setInventory} state={state} onSubmit={saveInventory} onClose={() => setInventoryFormOpen(false)} /> : null}
    </div>
  );

  return (
    <div className="flip-record-workspace">
      <section className="flip-section"><SectionHeading eyebrow="Stock on hand" title="Inventory" detail="Track item identity, quantity, cost allocation, storage, resale projections, and eventual sale history." actions={<button type="button" className="primary-button" onClick={() => { setInventoryForm(blankInventory()); setInventoryFormOpen((open) => !open); }}>{inventoryFormOpen ? "Close form" : "Add Inventory"}</button>} />{inventoryFormOpen ? <InventoryForm form={inventoryForm} set={setInventory} state={state} onSubmit={saveInventory} onClose={() => setInventoryFormOpen(false)} /> : null}{message ? <p className="flip-form-message" role="status">{message}</p> : null}</section>
      <section className="flip-section"><div className="flip-filter-bar"><TextInput label="Search inventory" value={query} onChange={setQuery} /></div>{visibleInventory.length ? <div className="flip-record-list">{visibleInventory.map((item) => <article className="flip-record-card" key={item.id}><div className="flip-record-card__head"><div><span>{item.productClassification}</span><h3>{item.name}</h3></div><StatusPill tone={item.status === "Sold" ? "muted" : "tide"}>{item.status}</StatusPill></div><div className="flip-record-facts"><span>Qty <strong>{item.quantity}</strong></span><span>Allocated <strong>{formatCurrency(item.allocatedItemCost)}</strong></span><span>Projected mid <strong>{formatCurrency(item.projectedResaleMid)}</strong></span><span>{item.storageLocation || "Location not set"}</span></div><p>{[item.pokemonName, item.setName, item.cardNumber, item.condition, item.gradingCompany && `${item.gradingCompany} ${item.grade}`].filter(Boolean).join(" · ") || "No card-specific details recorded."}</p><RecordActions onEdit={() => { setInventoryForm({ ...blankInventory(), ...item }); setInventoryFormOpen(true); window.scrollTo?.({ top: 0, behavior: "smooth" }); }} onDelete={() => onDelete("inventory", item.id, item.name)} /></article>)}</div> : <EmptyState title="No inventory matches">Add a real item or adjust the search. Creating a sale draft does not remove inventory.</EmptyState>}</section>
    </div>
  );
}

function InventoryForm({ form, set, state, onSubmit, onClose }) {
  return <form className="flip-form" onSubmit={onSubmit}>
    ...
    <FormActions><button type="submit" className="primary-button">{form.id ? "Update inventory" : "Save inventory"}</button><button type="button" className="secondary-button" onClick={onClose}>Cancel</button></FormActions>
  </form>;
}
