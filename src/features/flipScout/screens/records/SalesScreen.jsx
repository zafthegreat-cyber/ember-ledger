...
import { calculateSaleResults, nonNegative } from "../../calculations.js";
...
export default function SalesScreen({ state, onSave, onDelete }) {
  ...
  const suggestedCogs = selectedItem && nonNegative(selectedItem.quantity) > 0
    ? (nonNegative(selectedItem.allocatedItemCost || selectedItem.totalPurchaseCost) / nonNegative(selectedItem.quantity)) * nonNegative(form.quantitySold)
    : 0;
  const result = useMemo(() => calculateSaleResults({ ...form, allocatedCostOfGoodsSold: form.allocatedCostOfGoodsSold === "" ? suggestedCogs : form.allocatedCostOfGoodsSold }), [form, suggestedCogs]);
  ...
  const save = (event) => {
    ...
    const record = {
      ...form,
      allocatedCostOfGoodsSold: form.allocatedCostOfGoodsSold === "" ? suggestedCogs : form.allocatedCostOfGoodsSold,
      netProceeds: result.netProceeds,
      realizedProfit: result.realizedProfit,
      realizedRoi: result.realizedRoi,
    };
    onSave("sales", record, { title: form.id ? "Sale updated" : form.status === "Draft" ? "Sale draft saved" : "Sale recorded", detail: `${selectedItem?.name || "Inventory record"} · ${form.quantitySold} sold` });
    ...
    <section className="flip-section">
      ...
      {formOpen ? <form className="flip-form" onSubmit={save}>
        <div className="flip-form-grid">
          ...
          <MoneyInput label="Allocated cost of goods sold" helper={suggestedCogs ? `Suggested from allocated item cost: ${formatCurrency(suggestedCogs)}` : "Enter the cost basis assigned to the quantity sold."} value={form.allocatedCostOfGoodsSold} onChange={set("allocatedCostOfGoodsSold")} />
