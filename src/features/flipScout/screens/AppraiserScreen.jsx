  ...
  return (
    <div className="flip-screen flip-analysis-workflow">
      <section className="flip-section">
        <SectionHeading eyebrow="Guided analysis" title="Deal Analysis" detail="Recommendations use only the assumptions you enter. They are not purchase instructions or live price claims." />
        ...
        <form className="flip-form" onSubmit={save}>
          ...
          {step === 4 ? <section className="flip-analysis-step ops-decision-panel" aria-labelledby="analysis-decision-title">
            <div className="flip-result-title"><span>Step 5 of 5 · Recommendation</span><StatusPill tone={recommendationTone(result.label)}>{recommendation}</StatusPill><h2 id="analysis-decision-title">{form.title || "Current assumptions"}</h2><p>{result.explanation}</p></div>
            <div className="flip-decision-priority"><article><span>Maximum offer</span><strong>{formatCurrency(result.maximumRecommendedOffer)}</strong><small>Base purchase price after non-purchase acquisition costs</small></article><article><span>Landed cost</span><strong>{formatCurrency(result.landedCost)}</strong><small>All recorded acquisition costs</small></article></div>
            <div className="flip-result-metrics"><article><span>Break-even resale</span><strong>{result.breakEvenResalePrice === null ? "Unavailable" : formatCurrency(result.breakEvenResalePrice)}</strong></article><article><span>Maximum purchase price</span><strong>{formatCurrency(result.maximumBasePurchasePrice)}</strong></article></div>
            ...
            <div className="flip-rule-checks"><div className={result.meetsProfit ? "is-pass" : "is-fail"}><span aria-hidden="true">{result.meetsProfit ? "✓" : "!"}</span><p><strong>Minimum profit</strong><small>{result.meetsProfit ? "Meets rule" : "Does not meet rule"}</small></p></div><div className={result.meetsRoi ? "is-pass" : "is-fail"}><span aria-hidden="true">{result.meetsRoi ? "✓" : "!"}</span><p><strong>Minimum ROI</strong><small>{result.meetsRoi ? "Meets rule" : "Does not meet rule"}</small></p></div></div>
            ...
            <div className="flip-assumption-note"><strong>Risk flags and missing information</strong><p>{result.riskFlags.length ? result.riskFlags.join(" · ") : "No risk flags were entered. That does not mean the listing is risk-free."}</p><small>{missingInformation.length ? `Missing: ${missingInformation.join(", ")}.` : "Core calculation assumptions are present."}</small></div>
            <div className="flip-calculation-explanation"><h3>How this was calculated</h3><p>Net proceeds subtract selling fees, outbound shipping, packaging, reserves, and other selling costs. The maximum landed cost must satisfy both your minimum-profit and minimum-ROI rules. The maximum offer then subtracts acquisition costs other than the base purchase price.</p></div>
          </section> : null}
          ...
          <FormActions>
            ...
            {step < 4 ? <button type="button" className="primary-button" onClick={() => setStep((current) => Math.min(4, current + 1))}>Continue</button> : <button type="submit" className="primary-button">Save decision</button>}
