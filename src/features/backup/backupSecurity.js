...
const PROHIBITED_KEY_FRAGMENT_PATTERN = /(?:password(?:hash)?|passwordreset(?:link|token)|passphrase|secret|managedreference|apikey|privatekey|otp(?:code|token)?|oauth(?:code|state)|authorizationcode|codeverifier|securitycode|loginlink|rawmessage(?:body|content)|rawprovider(?:payload|request|response|logs?)|raw(?:payload|request|response|logs?)|protectedcontent|(?:credit|debit|payment)cardnumber|primaryaccountnumber|pan|cvv|cvc|credentials?|proxy(?:username|password|auth|authentication|url|endpoint|host|hostname|ip|ipaddress)|captcha(?:response|token)?|verificationcode)$/i;
